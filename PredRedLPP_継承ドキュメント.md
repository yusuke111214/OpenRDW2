...
### 🔴 問題10: 最大曲率時にアバターが直進する問題（2026-01-09発見・調査中）
...
**最新の調査（2026-01-10）:**
- **問題10の根本原因の調査** 🔴
  - **問題の本質**: なぜ緩い曲率では正常に動作し、最大曲率（0.133 rad/m）では直進するのか？
  - **重要な前提**: OpenRDW2は完全なシミュレーション環境であり、これは純粋にシミュレーションロジックの問題
  - **並進ゲイン削除テストの結果**:
    - 並進ゲインとNull actionを完全に削除してもアバターは直進する ❌
    - **確定した事実**: 問題の原因はアクション選択ではなく、**選択したアクションの適用処理**にある
  - **考えられる原因**:
    1. 回転の上書き/リセット（ApplyGains後に何らかの処理が回転をリセット）
    2. シミュレーターの状態管理（KeyboardController等の特別な挙動）
    3. 座標系の問題（最大曲率時のみ座標変換に問題）
    4. フレーム間の処理（適用した回転が次フレームまでに失われる）
  - **ステータス**: 根本原因未特定、アクション選択ではなく適用処理の問題と確定

**残課題:**
- **問題10の根本原因の特定と解決** 🔴
  - 最大曲率時にアバターが直進する問題（適用処理の問題）
  - **並進ゲイン削除テストで原因が絞り込まれた**: アクション選択は正常、適用処理に問題
  - さらなる調査が必要：
    1. **フレーム間の transform.rotation 追跡**（最優先）
       - ApplyGains()実行直後の値
       - 次フレーム開始時の値
       - いつどこで回転が失われるかを特定
    2. **最大曲率時と緩い曲率時の処理フロー比較**
       - 全く同じログを出力して一行一行比較
       - 最大曲率時のみ発動する処理を特定
    3. **他のコンポーネントの調査**
       - KeyboardController の Update/LateUpdate
       - MovementManager の座標更新処理
       - Redirector の継承とオーバーライド関係
