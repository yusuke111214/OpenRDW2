...
### 🔴 問題7: 時計回り・反時計回りの非対称性（2026-01-08発見・修正）
...
**緑の軌跡（最適軌跡 T_pred）が表示されない問題:**

この問題は、曲率の符号とは独立した別の原因がある可能性があります。追加されたデバッグログにより、以下を確認できます：
1. `T_pred` が null かどうか
2. `T_pred.points.Count` が 0 かどうか
3. 選択された bestAction の種類とコスト

**可能性のある原因:**
- `FilterFeasibleTrajectories` で全ての軌跡が除外されている
- `SelectBestTrajectoryUsingSimilarityMeasure` が null を返している
- 可視化の座標変換（物理空間→仮想空間）に問題がある
- LineRenderer の設定（色、幅、マテリアル等）に問題がある

デバッグログを確認することで、これらの原因を特定できます。
