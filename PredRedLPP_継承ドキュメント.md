...
**ログ分析結果:**
```
[PredRedLPP] Applying Curvature: -0.195, isWalking=False, deltaPos.magnitude=0.000
[Redirector] SetCurvature: NOT APPLIED (isWalking=false)

[PredRedLPP] Applying Curvature: -0.195, isWalking=True, deltaPos.magnitude=0.035
[Redirector] SetCurvature: input=-0.195, clamped=-0.195, deltaPos=0.035, rotationGC=-0.40°, applied=True
```

**根本原因:**
1. **isWalking=False の時**: 曲率が適用されない（`deltaPos.magnitude=0.000`）
2. **isWalking=True の時**: 適用されるが `rotationGC=-0.40°` という微小な回転

**計算:**
```
rotationGC = deltaPos.magnitude × curvature × Rad2Deg
          = 0.035m × 0.195 rad/m × 57.3
          = 0.39°/frame
```

1フレームあたりの移動量（0.035m ≈ 約50fps）が小さすぎるため、急激な曲率（0.195 rad/m）でも回転角度が微小（0.4°/frame）になってしまう。

60フレームで約24°の回転 → ユーザーには「ほぼ直進」に見える。

#### 修正内容（2026-01-09）

**GlobalConfiguration.cs:111**
```csharp
// 修正前
public float CURVATURE_RADIUS = 7.5f;

// 修正後
public float CURVATURE_RADIUS = 3.0f;  // 急激な旋回を可能にするため
```

**効果:**
- 最大曲率: `1/3.0 = 0.333 rad/m` （修正前: `1/7.5 = 0.133 rad/m`）
- 半径3mの円を描く（修正前: 半径7.5mの円）
- 同じ移動量での回転角度が2.5倍に増加
- `rotationGC = 0.035 × 0.333 × 57.3 = 0.67°/frame` → より目に見える旋回

**動作確認結果（期待値）:**
- 急激な旋回が視覚的に認識できるようになる
- 緩やかな旋回も引き続き正常に動作
- 両方向（時計回り・反時計回り）の旋回が正常に動作

**ステータス:** 🟢 解決済み（CURVATURE_RADIUS を 7.5m → 3.0m に変更）
