...
{
    public static readonly float INF = 100000;
    ...
    public static float GetSignedAngle(Vector2 prevDir, Vector2 currDir)
    {
        return GetSignedAngle(UnFlatten(prevDir), UnFlatten(currDir));
    }

    public static Vector3 GetInverseRelativePosition(Vector3 pos, Transform origin)
    {
        return origin.rotation * pos + origin.position;
    }
    ...
    // return 0 if the point is on line{p,q}, 1 if the point is on leftside of line{p,q}, 2 if the point is on rightside of line{p,q}
    public static int PointLineRelation(Vector2 point, Vector2 p, Vector2 q)
    {
        var s = (point.x - q.x) * (p.y - q.y) - (point.y - q.y) * (p.x - q.x);
    ...
    public static float Orient(Vector2 p1, Vector2 p2, Vector2 p3)
    {
        return Mathf.Sign((p2.x - p1.x) * (p3.y - p1.y) -
                          (p2.y - p1.y) * (p3.x - p1.x));
    }

    public static bool IfTwoLinesIntersect(Vector2 p1, Vector2 p2, Vector2 q1, Vector2 q2)
    {
        return ((Orient(p1, p2, q1) != Orient(p1, p2, q2)) &&
                (Orient(q1, q2, p1) != Orient(q1, q2, p2)));
    }

    public static float RayIntersectDistance(Vector2 pos, Vector2 dir, Vector2 s1, Vector2 s2)
    {
        var p1 = pos - s1;
        var p2 = s2 - s1;
        var p3 = new Vector2(-dir.y, dir.x);

        float d = Vector2.Dot(p2, p3);
        if (Mathf.Abs(d) < EPS)
            return INF;

        float t1 = Utilities.Cross(p2, p1) / d;
        float t2 = Vector2.Dot(p1, p3) / d;
    ...
    public static float PointLineDistance(Vector2 pos, Vector2 s1, Vector2 s2)
    {
        float t = Vector2.Dot(pos - s1, s2 - s1) / ((s2 - s1).magnitude * (s2 - s1).magnitude);
        t = Mathf.Min(Mathf.Max(t, 0), 1);
        var closest = s1 + ((s2 - s1) * t);
        return (closest - pos).magnitude;
    }
    public static Vector2 PointLineProjection(Vector2 pos, Vector2 s1, Vector2 s2)
    {
        float t = Vector2.Dot(pos - s1, s2 - s1) / ((s2 - s1).magnitude * (s2 - s1).magnitude);
        t = Mathf.Min(Mathf.Max(t, 0), 1);
        return s1 + ((s2 - s1) * t);
    ...
    public static float GetNearestDistToTrackingSpace(List<Vector2> trackingSpace, Vector2 currPosReal)
    {
    ...
    // Get nearest distance between currPosReal and other avatars
    public static Vector2 GetNearestAvtarPos(List<GameObject> redirectedAvatars, int avatarId, int physicalSpaceIndex, Vector2 currPosReal)
    {
        Vector2 ret = Vector2.zero;
        ...
        for (int i = 0; i < redirectedAvatars.Count; i++)
        {
            if (i != avatarId)
            {
                var dis = (currPosReal - FlattenedPos2D(redirectedAvatars[i].GetComponent<RedirectionManager>().currPosReal)).magnitude;
                if (dis < mindis)
                {
                    mindis = dis;
                    ret = FlattenedPos2D(redirectedAvatars[i].GetComponent<RedirectionManager>().currPosReal);
                }
            }
        }
        return ret;
    ...
    public static Tuple<float, Vector2> GetNearestDistAndPosToObstacleAndTrackingSpace(List<SingleSpace> physicalSpaces, int physicalSpaceIndex, Vector2 currPos)
    {
        var nearestPosList = new List<Vector2>();
        ...
        return new Tuple<float, Vector2>(dis, pos);
    }
